...
class CPUDataTest : public testing::Test {
  protected:
    CPUDataTest() {
        _vm = vm_new();
...
TEST_F(CPUDataTest, Instr_MOV_RR) {
...
TEST_F(CPUDataTest, Instr_MOV_VR) {
    _vm->regs_gp[0] = 0xDEADBEEF;
...
TEST_F(CPUDataTest, Instr_STR_RI0) {
...
TEST_F(CPUDataTest, Instr_STR_RV0) {
    constexpr uint32_t mem_addr = 12;
    static_assert(mem_addr + 4 <= RAM_SIZE);
...
TEST_F(CPUDataTest, Instr_STR_RI8) {
    constexpr uint32_t mem_addr = 12;
...
TEST_F(CPUDataTest, Instr_STR_RI32) {
    constexpr uint32_t mem_addr = 12;
...
TEST_F(CPUDataTest, Instr_STR_RIR) {
    constexpr uint32_t mem_addr = 12;
...
TEST_F(CPUDataTest, Instr_LDR_RV0) {
    constexpr uint32_t mem_addr = 12;
...
TEST_F(CPUDataTest, Instr_LDR_RI0) {
    constexpr uint32_t mem_addr = 12;
    constexpr uint32_t mem_dword = 0xCAFEBABE;
    static_assert(mem_addr + 4 <= RAM_SIZE);
...
TEST_F(CPUDataTest, Instr_LDR_RI8) {
    constexpr uint32_t mem_dword = 0xCAFEBABE;
...
TEST_F(CPUDataTest, Instr_LDR_RI32) {
    constexpr uint32_t mem_dword = 0xCAFEBABE;
    constexpr uint32_t mem_base = 12;
...
TEST_F(CPUDataTest, Instr_LDR_RIR) {
    constexpr uint32_t mem_dword = 0xCAFEBABE;
    constexpr uint32_t mem_base = 12;
    constexpr int32_t mem_off = -2;
    static_assert(mem_base + mem_off + 4 <= RAM_SIZE);
